...
class Google_ClientTest extends BaseTest
{
  ...
  private function checkAuthHandler($http, $className)
  {
    if ($this->isGuzzle6()) {
      ...
      if (is_null($className)) {
        ...
        $authClass = sprintf('GoogleAnalytics\Auth\Middleware\%sMiddleware', $className);
      ...
      if (is_null($className)) {
        ...
        $authClass = sprintf('GoogleAnalytics\Auth\Subscriber\%sSubscriber', $className);
  ...
  public function testApplicationDefaultCredentials()
  {
    ...
    $this->checkCredentials($http, 'GoogleAnalytics\Auth\Credentials\ServiceAccountCredentials');
  }
  ...
  public function testApplicationDefaultCredentialsWithSubject()
  {
    ...
    $this->checkAuthHandler($http, 'AuthToken');
    $this->checkCredentials($http, 'GoogleAnalytics\Auth\Credentials\ServiceAccountCredentials', $sub);
  }
